...
(function () {
  ...
  const projects = [
    ...
    { category: 'personal project', title: 'Retermina',
      description: 'An AI-coding terminal workspace built on Tauri v2 with a Rust backend driving native PTY shells — fully local, with no cloud, token limits, or subscription. An embedded Claude Code agent (unified Agent + CLI views with per-project token tracking) is the primary surface, while tools — split terminals, syntax-highlighted code with explorer, project-wide git diff, localhost tracker, and a native preview — slide in from a side drawer or fan out onto an optional draggable canvas, all re-skinnable instantly by five structural theme engines and portable Loom presets. Shells run in an out-of-process host that outlives the app, so terminals and their live dev servers survive a quit, crash, or auto-update and reattach on reopen — and the Claude session resumes right where it left off.',
